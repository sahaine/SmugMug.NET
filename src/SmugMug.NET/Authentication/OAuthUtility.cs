...
namespace SmugMug.v2.Authentication
{
    ...
    internal class OAuthUtility
    {
        ...
        /// </summary>
        internal static string GetAuthorizationHeader(OAuthToken tok, string endpoint)
        {
            List<KeyValuePair<string, string>> parameters = new List<KeyValuePair<string, string>>()
            {
                new KeyValuePair<string,string>("oauth_consumer_key", tok.ApiKey),
                new KeyValuePair<string,string>("oauth_token", tok.Token),
                new KeyValuePair<string,string>("oauth_signature_method", "HMAC-SHA1"),
                new KeyValuePair<string,string>("oauth_timestamp", GenerateTimestamp()),
                new KeyValuePair<string,string>("oauth_nonce", GenerateNonce()),
                new KeyValuePair<string,string>("oauth_version", "1.0"),
                new KeyValuePair<string,string>("oauth_token_secret", tok.TokenSecret) 
            };


            string baseString = OAuthUtility.GenerateBaseString(endpoint, "POST", parameters);
            string sig = OAuthUtility.EncodeValue(OAuthUtility.GenerateHMACDigest(baseString, tok.TokenSecret, tok.Secret));
